...
userSpeeds = [-5,-4,-3,-2,-1,-0.75,-0.5,-0.25,-.1,-.05,.05,.1,.25,.5,.75,1,2,3,4,5];#canon values for SrF
...
#=
#displacementsInMM = [1,2,3,5,7,9,11,14,17]; #for generic case
displacementsInMM = [0.5, 1, 1.5, 2, 3, 4, 5, 6, 7.5, 9, 10.5, 12, 14, 16]; #for SrF case
userSpeeds = [.05,.1,.2,.4,.6,.8,1,1.5,2,2.5,3,3.5,4]; #for SrF case 
...
laserEnergy = [-1.1,-9.8,-18.6,-26.8,-20.8, -stateEnergiesGround[1] + 0,-stateEnergiesGround[2] + 0, -stateEnergiesGround[3] + 0, -stateEnergiesGround[4] + 0]; #vib repump on resonance w/all levels
polSign = [1,1,1,-1,-1,1,1,1,1];
whichTransition = ["XA","XA","XA","XA","XA","XARepump","XARepump","XARepump","XARepump"];#
...
polSign = [1,1,1,-1,1,1,1,1];
whichTransition = ["XA","XA","XA","XA","XARepump","XARepump","XARepump","XARepump"];#
